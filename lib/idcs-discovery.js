...
var debug = require('debug')('idcs-discovery');
...
exports.getOAuthAccessToken = function(config, code, idcsAgent){
  debug('get Tokens config:', config);
  return new Promise(function(resolve, reject){
    var options = {
        method: "POST",
        ...
        body: "grant_type=authorization_code&code=" +code +"&scope=" +config.scope,
        agent: idcsAgent
      };
      request(options, function(err, res, body){
      if(err){
      ...
      debug('getOAuthAccessToke res:', res);
      var token = JSON.parse(body);
      debug('get Tokens:', token);
      if(token){
        resolve(token);
      }else{
